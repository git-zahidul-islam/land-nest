...
const images = [
  {
    ...
    stats: { views: "1.2k", likes: "350", location: "New York" },
  },
  {
    ...
    <section className="mb-10">
      <div className="md:flex justify-center relative">
        <div className="relative w-full md:h-[720px] h-[420px] overflow-hidden">
          ...
          {/* stats is here */}
          <div className="absolute bottom-5 right-5 text-white bg-black bg-opacity-50 p-1 rounded-lg flex md:flex-col flex-row md:gap-0 gap-3 md:justify-end justify-center">
            <p>Views: {images[currentIndex].stats.views}</p>
            <p>Likes: {images[currentIndex].stats.likes}</p>
            <p>Location: {images[currentIndex].stats.location}</p>
          </div>
        ...
        <section className="md:w-[80%] w-full mx-auto z-50 absolute md:top-1/2 top-[30%] md:rounded-s-2xl md:rounded-e-2xl md:px-0 px-5">
          <div className="flex items-center justify-center md:flex-row flex-col">
            <input
              className="md:text-lg text-base md:w-[40%] w-full md:rounded-s-2xl p-[11px] bg-opacity-80 bg-white outline-none"
              placeholder="search"
              type="search"
              name="search"
              id="search"
